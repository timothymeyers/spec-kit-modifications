...
# precedence over --number (consume only: the issue must already exist). The
# aligned number is never auto-corrected on conflict, so it is not treated as
# an explicit --number preference.
